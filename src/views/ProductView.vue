  ...
  <div class="product-details">
    ...
    <div class="item" v-for="product in myProduct" :key="product.id">
      <div id="img-item" class="item-img">
        <img id="img-1" :src="getImageUrl(product.firstImg)" alt="" />
      </div>
      ...
      <div class="item-details">
        ...
        <div class="quantity">
          ...
          <div class="quantity-btns">
            <button @click="product.count > 1 ? product.count-- : true">
    ...
    <div class="details">
      <button
        ...
        "
        :style="{ backgroundColor: selectedDiv === 1 ? 'var(--yellow)' : '' }"
      >
        DESCRIPTION
      </button>
      ...
      <button
        ...
        "
        :style="{ backgroundColor: selectedDiv === 2 ? 'var(--yellow)' : '' }"
      >
        REVIEWS
      </button>
    ...
    <ProductSwiper text="Popular Products" :products="popularProducts" />
  </div>
</template>
...
const detailsActive = ref(true)
const myProduct = ref([])
const selectedDiv = ref(1)
...
//   store.commit('getPopularProducts')
//   popularProducts.value = store.state.popularProducts
// }

// console.log('THE ID', route.params.id)

const getMyProduct = () => {
  myProduct.value = store.state.allProducts.filter(
    (e) => e.id == route.params.id,
  )
  // console.log('PROD', myProduct.value)
}

watch(() => route.params.id, getMyProduct, { immediate: true })
...
const addItemToCart = (product) => {
  ...
  for (let i = 0; i < store.state.cart.length; i++) {
    if (store.state.cart[i].id === product.id) {
      store.state.cart[i].count += product.count
      exists = true
      break
    }
  }
